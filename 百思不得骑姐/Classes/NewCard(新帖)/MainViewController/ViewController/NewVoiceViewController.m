...
@interface NewVoiceViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic, strong) UITableView *tableViewAll;
@property (nonatomic, strong) NSMutableArray *dataArray;
@property (nonatomic, copy) NSNumber  *maxTime;
@end

@implementation NewVoiceViewController
...
- (void)loadData
{
    ...
    [manager GET:NEWCardVoice parameters:nil success:^(AFHTTPRequestOperation *operation, NSDictionary *responseObject) {
...
- (void)setUpMoreData
{
    ...
    MJRefreshAutoNormalFooter *footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            count++;
            AFHTTPRequestOperationManager *manager = [[AFHTTPRequestOperationManager alloc] init];
            NSString *str = @"http://api.budejie.com/api/api_open.php?a=newlist&appname=baisishequ&asid=56280F3D-FFF0-4D6A-ABA9-C52F3D13282B&c=voice&client=iphone&device=iPhone%";
            NSString *str1 = [NSString stringWithFormat:@"204S&from=ios&jbk=0&mac=&market=&maxtime=%@&openudid=379ec5e20a2a6ca41a523ed24c985a847b0a580a&page=%d&per=20&sub_flag=1&type=31&udid=&ver=3.6.1",self.maxTime,count];
...
//    NSString *identifier = [NSString stringWithFormat:@"%ld%ld",indexPath.section,indexPath.row];
    static NSString *identifier = @"NewVOICE";
