...
//    _topScrollView.backgroundColor = kUIColorFromRGB(0xF7F7F7);
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
         CGRect rect = [vc.title boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:kFontSizeOfTabButton]} context:nil];
        CGSize textSize = rect.size;
        //累计每个tab文字的长度
        topScrollViewContentWidth += kWidthOfButtonMargin+textSize.width;
        //设置按钮尺寸
        [button setFrame:CGRectMake(xOffset + 20,0,
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin + 20, 0, textSize.width, _shadowImage.size.height);
            button.selected = YES;
        }
